...
function task(overrides: Partial<TaskItemDto>): TaskItemDto {
  return {
    id: "task", title: "Task", description: "", done: false,
    priority: "none", dueAt: null, remindAt: null, sortOrder: 0,
...
describe("buildTodoOverview", () => {
  ...
  it("summarizes progress and date pressure", () => {
    const overview = buildTodoOverview([
      task({ id: "1", title: "Overdue", dueAt: "2026-01-01" }),
      task({ id: "2", title: "Today", dueAt: "2026-01-02" }),
      task({ id: "3", title: "Done", done: true, dueAt: "2026-01-01" }),
    ], today);
  ...
  it("keeps high priority focus and falls back to upcoming work", () => {
    const overview = buildTodoOverview([
      task({ id: "1", title: "Upcoming", dueAt: "2026-01-04" }),
