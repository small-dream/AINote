...
import type { SyncTone } from "../utils/status";
...
export function SyncBar({ repoPath }: SyncBarProps) {
  const { online, label, syncNow, isSyncing, status, resolve, resolving } = useSync(repoPath);
  const [conflictOpen, setConflictOpen] = useState(false);

  return (
    <div className="flex items-center justify-between border-b border-bg-secondary px-4 py-2">
      <div className="flex min-w-0 items-center gap-2">
        <span className={`h-2 w-2 shrink-0 rounded-full ${TONE_DOT[label.tone]}`} />
        <span className="truncate text-sm text-text-secondary">{label.text}</span>
