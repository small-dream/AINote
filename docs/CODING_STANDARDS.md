...
- 文本输入框、文本域和下拉框必须只有一层可见边框：聚焦态使用控件自身的 `border` 颜色变化，禁止同时叠加全局 `outline`、`ring` 或第二层 `border`，避免出现同心双边框。
- 采用外层容器承载边框的搜索框等组合控件，内部输入必须保持 `bg-transparent`、无边框、无 outline，聚焦态统一由外层 `:focus-within` 表达。
- 新增或修改输入控件必须复用现有控件样式语义，并检查亮色、暗色、桌面与移动端聚焦态；不得通过单个页面的特殊覆盖绕过该约束。
...
错误码规范: <域>_<序号>  →  NOTE_1xxx, AUTH_2xxx, REPO_3xxx, SYNC_4xxx, GIT_4xxx, IO_5xxx, AI_6xxx, TASK_8xxx
```
