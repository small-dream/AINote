...
**M1 范围**：23 个任务中 19 个在范围内；范围外 4 个 = `E1-T1` / `E1-T2` / `E1-T4`（签名与 iOS，🧊 暂缓到 M1.5）+ `E2-T4`（远程上报，🧊 等 D2 决策，本阶段只做本地日志与手动诊断包）。
...
- **跨端影响**：`shared`。
- **实现备注（2026-09-10）**：
  - 开关落在应用配置（`ainote.json` 的 `metricsEnabled`，缺省视为开启，与已确认的 D4 一致）：`config::metrics_enabled` / `config::set_metrics_enabled`。「关闭后停止写入」只有一个入口——`metrics_service::record_if_enabled(store, enabled, event)`，关闭时直接返回、不创建文件；`record_best_effort` 与 `metrics_record` 命令都走它，新增命令 `metrics_set_enabled`。
  - 设置页「诊断与反馈」新增 `MetricsCard`：首次打开设置时用 `role="status"` 展示一次说明（本机 localStorage 记忆，localStorage 不可用时视为已读、不反复打扰），卡片内含本地开关、「清空本机计数」（二次确认，与开关无关）、已记录次数，以及「没有远程上报」的明示；文案与实现逐条对齐（不含内容 / 路径 / 远端地址 / 凭证，无远程上报）。
  - 新增 `docs/PRIVACY.md`：采集范围（7 个白名单事件 + 平台与版本）、存储位置（各平台 `metrics.json` 路径）、关闭方式、删除方式、与日志 / 诊断包的关系，并明确「当前版本没有任何远程上报」；设置页与 README 的隐私入口都指向该文档。远程上报本身仍属 M1b（`M1-E2-T4`，等 D2）。
  - 验证：Rust `record_if_enabled` 关闭时「不创建文件、不写入」+ `dto` 开关状态共 2 项新增（全量 217）；前端 `MetricsCard` 6 项（首次说明与记忆、已读不再提示、关闭开关调用 IPC 与提示、关闭态回显、确认后清空、取消不清空）；e2e 新增 `e2e/privacy-metrics.spec.ts` 4 条（桌面：首次说明一次且整页重载后不再出现、关闭开关 + 清空、关闭态经 IPC 回显；窄屏 430px：说明与开关可用），IPC mock 支持指标开关与计数状态。
...
- **跨端影响**：`shared`。
- **实现备注（2026-09-10）**：
  - 漏斗口径落在纯函数 `src/features/settings/utils/metricsFunnel.ts`：安装（`app_launched` 计数）→ 绑定仓库（`repo_bound` 计数 + 占启动率）→ 创建笔记（`note_created` 计数 + 占绑定率）→ 周活跃天数（`active_days_7d`）→ 同步成功率（`sync_success_rate_7d`）。转化率以「本环节计数 ÷ 上一环节计数」近似，累计计数会超过分母，因此展示封顶 100%（`FUNNEL_TARGETS` 与 ROADMAP §3.2 目标一一对应：绑定率 ≥ 85%、周活跃 ≥ 3 天、同步成功率 ≥ 99%）。
  - 如实标注口径缺口：ROADMAP 里「绑定 → 第一篇笔记」的目标是**首次创建耗时 < 3 分钟**，本机没有采集时长（新增事件需评审并更新 `docs/PRIVACY.md`），因此该环节只显示计数与转化率，判定列显示「无结论」；同步无样本时显示「—」而不是 0%，避免把「没用过」误读成「全失败」。
  - 导出走用户主动操作：`metrics_export(format)`（`json` / `csv`）复用诊断包的保存对话框选择器（抽出 `commands/save_file.rs::choose_destination`），用户取消返回 `None` 且不报错；导出内容由 `MetricsSnapshot::export_payload` / `to_csv` 生成，只有白名单计数、时间、平台与版本，结构上无法带出笔记内容或凭证。CSV 为 `events` + `summary` 两段，字段含逗号 / 引号 / 换行时按 RFC 4180 转义。
  - UI：`MetricsFunnel.tsx`（纯展示，i18n 映射 label / target / verdict）+ `MetricsActions.tsx`（导出 JSON / CSV、清空、已记录次数）；`MetricsCard` 只负责组装与开关，保持在 300 / 220 行约束内。
  - 验证：Rust 新增 6 项（导出 JSON 与 CSV 落盘内容、无样本成功率留空、CSV 转义、格式解析与扩展名、导出 DTO），全量 `cargo test` 223 通过；前端新增 `metricsFunnel` 10 项（换算、封顶、无样本、缺失事件按 0、格式化）+ `MetricsCard` 6 项（漏斗展示与无样本、导出 JSON / CSV 走 IPC、取消不提示、失败提示），全量 `pnpm test` 696 通过；e2e 新增 5 条（漏斗数值与达标、无样本不显示 0%、导出 JSON + CSV 两次 IPC、取消保存无提示、窄屏 430px 可看漏斗并导出），`npx playwright test` 29 通过。
  - 跨端：桌面（默认视口）与移动单栏壳（430px 窄屏）各验证一次；移动端仍以窄屏 e2e + RTL 替代真机验证（本机 `adb devices` 为空，无可用 Android 设备），后续真机回归需补。
...
| M1-E5-T2 同意页与开关 | ✅ | | 首次打开设置的一次性说明 + 本地开关（关闭即停止写入）+ 清空；新增 docs/PRIVACY.md，无远程上报 |
| M1-E5-T3 指标导出与漏斗 | ✅ | | 纯函数漏斗（口径对齐 ROADMAP §3.2，含目标对照与「无结论」）+ 导出 JSON / CSV（用户主动、取消不报错）；保存对话框逻辑抽为 commands/save_file.rs |
...
| 日期 | 变更 | 作者 |
|---|---|---|
| 2026-09-10 | v1.15：M1-E5-T3 交付（本机漏斗视图（安装 → 绑定仓库 → 创建笔记 → 周活跃天数 → 同步成功率，对照 ROADMAP §3.2 目标标注达标 / 无结论）+ 用户主动导出 JSON / CSV；保存对话框选择器抽为 `commands/save_file.rs` 共用；修正任务总览中「范围内任务数」与实际暂缓项不一致的问题） | PM |
| 2026-09-10 | v1.14：M1-E5-T2 交付（首次打开设置的计数说明 + 本地开关（关闭即停止写入）+ 清空入口；新增 `docs/PRIVACY.md` 并让设置页/README 指向它；明确当前无远程上报） | PM |
