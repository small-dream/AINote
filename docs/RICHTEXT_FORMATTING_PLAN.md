...
> 版本：v1.1 · 状态：方案已确认（决策见 §10，未进入实现）· 维护：产品与前端协作
...
## 4. Markdown 互通分级（本方案的关键增量）

评估结论是「导出必然丢样式」。逐项复核后，**高亮存在一条零语法成本的互通路径**，其余三项没有：

| 样式 | `.ainote` 内 | `.md` 互通 | 机制 |
|---|---|---|---|
| **高亮** | mark | **双向** | `==文本==`：Obsidian / Bear 的事实标准；本项目已有两处现成挂点（见下） |
...
这一分级让「Git 即数据库」的定位更完整：**高亮是唯一能在 `.md` 与 `.ainote` 之间无损往返的字符样式**，其余三项在笔记类型转换时显式提示损失。
...
### M2：Markdown 高亮互通
`==…==` 四条链路（富文本读写、软渲染、预览）+ 转换损失清单文案定稿。

退出标准：`.md` 写 `==重点==` 与 `.ainote` 设黄高亮互相转换后语义不丢；`.ainote → .md` 时颜色/字号/字体在转换对话框中被逐项列为损失；导出 Markdown 不残留任何 HTML 标签。
...
改动
  extensions.ts / RichTextToolbar.tsx / RichTextBubbleMenu.tsx / toolbarCommands.ts
  utils/markdownMarks.ts（高亮 inline 规则）、utils/conversionLoss.ts（新增损失项）
  src/features/note/utils/markdownPipeline.ts（remarkHighlight）
  src/features/note/softRender/utils/plan.ts（== 的 hides + marks）
  rich-text.css / softrender.css / src/styles/markdown-themes.css / src/features/export/export.css
  src/i18n/messages.ts（约 14 个新 key）、e2e/richtext-flow.spec.ts
```
...
4. 高亮往返：`==x==` ↔ mark ↔ `==x==` 三向断言；`.md` 导出不残留 HTML 标签。
5. `conversionLoss` + i18n：颜色 / 字号 / 字体被识别为损失项并展示。
6. `toolbarCommands`：四组动作的 `isActive` 与 `run`。
7. e2e：选中文字 → 设高亮与颜色 → 落盘 JSON 断言；Markdown 侧 `==` 软渲染与预览断言；桌面与移动视口各一遍。

## 8. 风险与缓解

| 风险 | 缓解 |
|---|---|
| 生产壳内联 `style` 是否被 CSP 拦截尚未实测 | 方案本身不依赖内联 style；M1 前在生产壳跑一次最小验证并把结论写回评估文档 §4.1 |
| `color-mix()` 在部分 WebView 版本不可用 | 提供常量兜底色 + `@supports` 分支，纳入 M1 验收 |
...
3. **`==高亮==` 互通按 M2 实施**（含软渲染与预览改动），使高亮成为唯一可跨 `.md` / `.ainote` 无损往返的字符样式。
