...
| 8 | **可度量性不完整** | 成功指标已定义，本机可看但无法跨用户验证 | 本机事件计数与漏斗已落地（E5-T1/T2/T3，可导出 JSON / CSV）；跨用户验证需 opt-in 远程上报（M1b / E2-T4） |
...
> 可执行任务清单见 `docs/M1_TRUST_PLAN.md`（23 个任务卡，其中 19 个在 M1 范围内；3 个签名 / iOS 任务 🧊 暂缓至 M1.5，`E2-T4` 远程上报 🧊 等 D2 决策）。
...
1. **编辑器能力收口**（对齐 `docs/EDITOR_EXPERIENCE_PLAN.md` 未完成项）
   ...
   - 查找替换 UI 完善；多行粘贴保持缩进（✅ 已实现：面板中文化与排版收口，覆盖 CodeMirror 内建 27 条英文短语；列表按内容起点、引用补 `>`，多行粘贴不再脱出容器）。
...
5. **搜索增强**
   - 结果高亮、类型/目录筛选、搜索历史。
6. **版本掌控（阶段 A + 阶段 B）**
   - 移除空闲 15 分钟自动 commit 与新建/导入/转换的即时 commit，改为用户手动提交 + 同步前自动兜底。
   - 手动提交面板：待提交变更（增/改/删 + 路径）+ 自动生成可编辑 message（时间 + 修改文件）；桌面导航轨常驻入口与移动端头部双端一致。
   - 同步前 commit message 改用同一规则生成；冲突解决提交保留。
   - 阶段 B Repo Git Graph：桌面导航轨与移动端列表 Tab 打开全仓提交历史（时间倒序，含每 commit 直接改动的文件 A/M/D）；选中提交查看文件列表与单文件 diff，支持「恢复此版本」写回工作区；不做分支创建/切换/revert/manual merge。
   - 丢弃本地改动（P1-19）：同一份待提交变更列表支持单选 / 多选 / 全选后按文件回滚——已修改 / 已删除恢复到 `HEAD`，新增文件彻底删除；二次确认需勾选方可执行，合并冲突中拒绝，与同步 / 提交共用写锁；桌面导航轨与移动端头部双端入口。
...
2. **后台同步（尽力而为）**
   ...
   - 移动端冲突处理体验对齐桌面（✅ 已实现：单栏分页签 + 底部固定操作条，一键保留一侧 / 完成合并并推送，失败原因与重试在面板内回显）。
3. **移动端能力补齐**
...
| 语义问答 | 中 | 高 | 中 | 高 | **M3 做** |
| 版本掌控 / Git 透明度（手动提交 + Repo Graph） | 高 | 中高 | 高 | 中 | **M2 已交付**（阶段 A 手动提交；阶段 B Repo Git Graph） |
...
| P0-7 设备级快速解锁（macOS / iOS / Android） | ✅ 已完成（默认关闭；详见 `docs/QUICK_UNLOCK_PLAN.md`） | — |
| P0-AI-1 ~ P0-AI-4 | ✅ 已完成 | — |
| P1-AI-1 ~ P1-AI-4 | ✅ 已完成 | — |
| P1-AI-5 标签与双链建议 | 📋 | M3 |
| P1-AI-6 代码解释 / 生成 | 📋 | M3 |
...
| P2-2 协作（多人实时） | 🧊 | M5 评估 |
| P2-3 插件系统 | 🧊 | M5 |
| P2-4 端到端加密仓库 | 🚧 加密笔记（远端静态加密，决策见 `ENCRYPTED_NOTES_PLAN.md`）已落地，快速解锁 P0-7 已交付；完整 E2E 加密仍 🧊 | M5 评估 |
| 编辑器体验计划未完成项 | 📋 | M2 |
