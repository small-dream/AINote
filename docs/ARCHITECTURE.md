...
- **Markdown 诊断（`features/diagnostics`）**：纯函数诊断（`utils/diagnostics.ts`，覆盖未闭合围栏、表格结构、frontmatter、图片引用）+ `useMarkdownDiagnostics` 合并异步断链结果（新命令 `asset_exists` 批量校验仓库相对文件存在性，非法路径视为不存在）；悬浮入口与大纲并列，点击问题项按行号定位到编辑器。诊断只读、不写库，供编辑时即时提示。
