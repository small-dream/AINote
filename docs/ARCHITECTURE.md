...
│   │   ├── todo/                  # Todo 清单（P1-16）：多清单面板 + 截止日期/优先级 + 到期提醒调度
...
- **Todo 清单（独立待办，P1-16）**：`features/todo` 提供类滴答清单的待办体系，与笔记完全解耦。数据落盘为仓库隐藏文件 `.ainote/todos.json`（`schemaVersion` + 原子写，照抄 favorites 模式），随仓库 Git 版本化与多端同步，被搜索 / wiki / 文件树扫描自动忽略；变更不做即时提交，走手动提交 / 同步兜底（高频勾选不刷提交历史）。后端 `domain/task.rs`（TaskBoard / TaskList / TaskItem DTO + 分组排序纯函数）→ `repositories/task_files.rs` → `services/task_service.rs` → `commands/task/`（board / create_list / rename_list / delete_list / create / update / toggle / delete 八个命令，错误码 TASK_8xxx）；前端 `task.queries`（key `["tasks", repoPath]`）→ `features/todo` 面板。任务采用「快捕 + 详情」双层交互：Quick Add 保持单行捕获并解析日期/优先级，可展开详情补充说明；桌面 `TodoWorkspace` 将清单/任务放在左侧，把同一详情字段放到主内容区右侧；移动端保留单栏行内编辑。数据当前为 `schemaVersion 2`，旧版缺失详情自动迁移为空文本。桌面入口为导航轨中紧跟「笔记」的「待办」项，移动端入口为底部导航与列表 Tab 的「待办」项。到期提醒由 `useTaskReminders` 经 `tauri-plugin-notification` 调度（`capabilities` 双端加 `notification:default`，首次开启提醒时才请求系统权限、拒绝时静默降级）：移动端由 OS 调度（应用挂起仍可触发），桌面端在应用运行期间触发、启动时对已过期未完成提醒补发「已到期」通知；默认提醒时间为截止当天 09:00 本地，无截止日期不允许设提醒（前后端双重校验）。多端并发编辑 todos.json 产生冲突时走既有冲突解决流，不做字段级合并。
