...
空闲自动锁定（P1，已实现）：设置项「自动锁定」默认 5 分钟，可选 1 / 5 / 15 / 30 分钟或「从不」（纯前端 UI 偏好，localStorage 持久化）。解锁入口已就地化（P0：笔记内嵌解锁表单、全局解锁弹层、桌面导航轨与移动端入口、Cmd+K 命令），输入口令是低摩擦动作，因此默认 5 分钟不会造成明显打扰。计时在纯前端完成：用户活动（输入 / 点击 / 滚动）重置；窗口隐藏 / 锁屏 / 睡眠期间清掉计时器，回到前台按「离开时长」补算——超过阈值立即锁定，否则按剩余时间继续；锁定前先落盘全部草稿，失败不锁定并 toast 提示。
