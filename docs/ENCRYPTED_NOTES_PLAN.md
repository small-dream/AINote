...
锁定 = 清空 `VaultSession`。桌面端锁定时机：**启动即锁定**（必须先解锁才能看加密笔记）、手动锁定、可配置空闲自动锁定。

空闲自动锁定（P1，已实现）：设置项「自动锁定」**默认「从不」**，可选 30 分钟 / 1 小时 / 2 小时或「从不」（纯前端 UI 偏好，localStorage 持久化）。默认从不是 v0.53.1 的修订：解锁入口虽然已就地化（P0：笔记内嵌解锁表单、全局解锁弹层、桌面导航轨与移动端入口、Cmd+K 命令），但主口令本身是高摩擦输入，早期「默认 5 分钟」在真实使用中表现为「离开一会儿回来就要重新输入一次口令」，与用户「一次解锁、本次运行内持续可用」的预期相悖，故默认改为从不；需要「离开电脑即保护」的用户显式开启，或改用设备级快速解锁。档位整体后移、短档位（1 / 5 / 15 分钟）一并移除——它们低于「离开电脑」场景的实际尺度，只会制造频繁解锁；存量偏好方面，显式选过 30 / 60 / 120 分钟的保持原状，已移除的 1 / 5 / 15 分钟就近落到最短的 30 分钟，从未设置过（含旧版默认值）一律解析为「从不」。计时在纯前端完成：用户活动（输入 / 点击 / 滚动）重置；窗口隐藏 / 锁屏 / 睡眠期间清掉计时器，回到前台按「离开时长」补算——超过阈值立即锁定，否则按剩余时间继续；锁定前先落盘全部草稿，失败不锁定并 toast 提示。
...
1. **口令强度校验**：`.ainote/vault.json` 可公开，安全性完全取决于口令强度，必须挡住离线暴力破解。规则：≥ 6 字符、拒绝与邮箱/用户名/仓库名高度相同的口令、建库时给出强度提示；这是唯一的防线，不做「弱口令但允许」的降级。
...
| E5 | 边界降级：AI 禁用、冲突二选一、版本历史入口整体关闭、锁定时机（启动即锁定 + 手动 + 可配置空闲自动锁定）、双端验证 |
...
**P0 解锁入口就地化（desktop + mobile + shared）**

- 笔记锁定遮罩内嵌解锁表单（`VaultLockedNote` 复用 `VaultUnlockCard`），解锁成功自动失效 Query 并重载当前笔记，不再跳转设置页；保留「在设置中管理」次级入口。
- 新增全局解锁弹层 `VaultUnlockDialog`（解锁成功自动关闭），由 `WorkspaceShellSwitcher` 统一挂载（双壳共用）。
- 桌面导航轨新增加密状态入口：已解锁（一键锁定，锁定前落盘草稿）/ 已锁定（点开解锁弹层），未建库不显示。
- 移动端 header 新增同语义入口（`MobileVaultButton`）；`Cmd+K` 命令面板按状态展示「解锁加密笔记 / 锁定加密笔记」命令。

**P1 空闲自动锁定（desktop + mobile 一致，纯前端计时，不触碰 Rust）**

- 设置页「加密笔记」新增「自动锁定」选项（`VaultAutoLockSettings`），偏好存 localStorage（与主题等 UI 偏好一致）。
- `useVaultAutoLock`：用户活动重置计时；`visibilitychange` 隐藏 / 锁屏 / 睡眠期间清计时，回到前台按离开时长补算；超时先 `flushPendingDrafts` 再 `vault_lock`，落盘失败 toast 提示并重新计时。
- 统一锁定动作收敛到 `useVaultLock`（设置页 / 导航轨 / 命令面板 / 自动锁定共用同一「先落盘再锁定」语义）。

影响面：`desktop`（导航轨、ShellSwitcher 挂载）、`mobile`（header 入口、计时同样生效）、`shared`（`VaultUnlockCard` 复用、`vault.queries`、`useVaultLock`、`useVaultAutoLock` 平台中立）。
