...
**关键业务规则**：上下文默认仅当前笔记，全库上下文只取关键词 top-k 段落（默认 5 段）；每次请求均由用户显式触发；AI 输出先预览再落笔，落笔后照常进入防抖保存与 Git 提交链路；回答渲染走既有 sanitize 管线。创作输出使用与当前笔记一致的 Markdown / 富文本语义；审查输出 Markdown 报告，只列问题、修改建议和无法核实项；优化在无选区时替换整篇笔记，有选区时替换选区，且必须经用户确认。
