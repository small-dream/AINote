...
| P0-2 | 作为用户，我可以创建、编辑、删除 Markdown 笔记 | Markdown 编辑 + 实时预览；自动保存（防抖）；新建笔记即时本地 commit（`note: create <path>`）；支持默认 / 每日 / 空白三种新建模板 |
...
- **自动提交策略**：新建笔记时立即生成一条 commit（message：`note: create <path>`）实现即时版本化；编辑内容防抖（默认 30s）落盘后，由「一键同步」统一提交（message：`note: auto commit`）并推送。
