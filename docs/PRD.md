...
| P0-2 | 作为用户，我可以创建、编辑、删除笔记 | 新建时可选「Markdown / 真富文本」类型：Markdown 默认 CodeMirror 软渲染（WYSIWYG）所见即所得编辑，标题/加粗/列表/任务/链接/图片/表格等直接渲染、光标所在元素淡显原始标记，`Ctrl+/` 可切回纯源码，并支持编辑/分栏/预览三种视图 + 同步滚动；富文本走 TipTap 所见即所得编辑（基础排版工具栏）；两种类型均支持自动保存（防抖）、新建笔记即时本地 commit（`note: create <path>`）、默认 / 每日 / 空白三种新建模板 |
...
- **Markdown 软渲染**：Markdown 编辑默认启用 CodeMirror 软渲染（Typora 式 WYSIWYG）——标题、加粗/斜体/删除线、行内代码、代码块、引用/Callout、列表与任务勾选、链接/双链、图片、表格、分割线均直接渲染；光标位于某元素内时淡显其原始标记以便就地编辑。源码 100% 原样保存与提交（Git 保真、全文搜索 / wiki 索引不受影响）。`Ctrl+/` 或工具栏按钮在「软渲染 / 源码」间切换，偏好按笔记持久化，默认软渲染。
