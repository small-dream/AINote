...
| P0-2 | 作为用户，我可以创建、编辑、删除笔记 | 新建时可选「Markdown / 真富文本」类型：Markdown 默认 CodeMirror 软渲染（WYSIWYG）所见即所得编辑，标题/加粗/列表/任务/链接/图片/表格等直接渲染；加粗/斜体/删除线/行内代码等行内样式编辑时保持渲染效果（不淡显标记），标题/链接/图片/表格等结构元素编辑时淡显原始标记以便就地修改（无需在源码/所见即所得间切换），并支持编辑/分栏/预览三种视图 + 同步滚动（分栏编辑侧为源码 + 实时预览）；富文本走 TipTap 所见即所得编辑（基础排版工具栏）；两种类型均支持自动保存（防抖）、新建笔记即时本地 commit（`note: create <path>`）、默认 / 每日 / 空白三种新建模板 |
...
- **Markdown 软渲染**：Markdown 编辑固定启用 CodeMirror 软渲染（Typora 式 WYSIWYG）——标题、加粗/斜体/删除线、行内代码、代码块、引用/Callout、列表与任务勾选、链接/双链、图片、表格、分割线均直接渲染；行内样式（加粗/斜体/删除线/行内代码）编辑时始终隐藏标记、保持渲染效果，标题/链接/图片/表格等结构元素在光标位于其中时淡显原始标记以便就地编辑。源码 100% 原样保存与提交（Git 保真、全文搜索 / wiki 索引不受影响）。编辑视图固定软渲染、无源码切换入口；分栏视图编辑侧为源码编辑 + 右侧实时预览。
...
- **新建模板**：新建笔记先选类型（Markdown / 真富文本），再选「默认 / 每日 / 空白」模板；默认文件名按类型带扩展名（`YYYY-MM-DD.md` / `YYYY-MM-DD.ainote`）；已存在同名笔记时给出提示并幂等打开已有文件。
