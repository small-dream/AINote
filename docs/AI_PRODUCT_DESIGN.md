...
| P1-AI-4 | 文字创作 / 笔记审查 / 笔记优化 ✅ 已实现：Markdown 与富文本 AI 菜单均提供「创作文字」「审查笔记」「优化笔记」。创作按标题和上下文产出结构化内容；审查事实、逻辑、结构与语法后输出修正后的完整笔记，确认后替换原文，不把审查报告插入正文；审查请求不会诱导联网或工具调用；优化结构、组织与表达，无选区时替换整篇，有选区时替换选区，全部先预览再确认落笔 |
| P1-AI-5 | 标签与双链建议（复用 wiki 索引与现有 `#标签` / `[[双链]]` 体系） |
| P1-AI-6 | 代码块解释 / 生成（识别代码语言上下文） |
...
- **输出落笔规则**
  - AI 结果先进入「待插入」预览态，用户确认后才写入编辑器；富文本经转换以 TipTap 语义写入，Markdown 直接替换原文 / 选区 / 光标。
  - 「优化笔记」在无选区时整篇替换，有选区时替换选中内容；「审查笔记」基于整篇笔记输出修订后的完整内容，确认后替换原文，不把审查报告追加到正文。
  - 落笔后自然进入现有防抖自动保存链路，AI 生成内容随下次手动提交/同步前兜底提交一起版本化，同样可回溯。
- **安全**
