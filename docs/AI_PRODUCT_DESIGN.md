...
| P1-AI-4 | 文字创作 / 笔记审查 / 笔记优化 ✅ 已实现：Markdown 与富文本 AI 菜单均提供「创作文字」「审查笔记」「优化笔记」。创作按标题和上下文产出结构化内容；审查事实、逻辑、结构与语法，输出 Markdown 报告，并明确列出无法核实项（若所选云端模型支持联网检索，可由该模型能力补充核查）；优化结构、组织与表达，无选区时替换整篇，有选区时替换选区，全部先预览再确认落笔 |
...
- **输出落笔规则**
  ...
  - 「优化笔记」在无选区时整篇替换，有选区时替换选中内容；「审查笔记」始终输出报告而不是直接改写正文。
