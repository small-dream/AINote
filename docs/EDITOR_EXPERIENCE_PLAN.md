...
- [x] 查找替换面板中文化与排版收口：覆盖 CodeMirror 内建英文文案（面板按钮、跳转行对话框、屏幕阅读器播报共 27 条短语），面板改为可换行工具条、输入框自适应宽度、关闭按钮独立定位，分栏与窄屏下不再被压扁。
- [x] Markdown 输入规则首版：无序/有序/任务列表续写，空列表项回车退出列表。（引用续写、代码围栏和更完整缩进策略留待后续 input rules PR）
- [x] 粘贴多行文本时保持缩进（列表项按内容起点补空格、引用补 `>`、两者叠加同时保留）；粘贴图片继续走资产导入管线。
...
- [x] 预览/富文本左侧大纲栏：正文限宽居中阅读，侧栏目录点击跳转标题。
...
**样式归属的四类边界**（新增，与字符级样式对齐）：改样式设计前先确认它属于哪一类，混用会导致「换个主题笔记就变了」或「文件里塞满非内容数据」。
