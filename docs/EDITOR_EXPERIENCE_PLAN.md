...
- 新增当前笔记大纲（H1–H6）和标题快速跳转。
- 输入 `[[` 时显示笔记目标补全；输入 `#` 时显示标签补全。
- 未解析双链使用明确但克制的视觉状态。
- 反向链接展示来源标题、上下文片段和跳转入口。
- 预览标题、双链和图片点击后保留回到原编辑位置的能力。
- 保存每篇笔记的编辑/预览模式、滚动位置和分栏比例。
- 扩展 Cmd/Ctrl+K：打开笔记、切换模式、查找替换、插入 Callout、保存版本、专注模式。
